...
> "​In this module, we are going to introduce you ​to the forward contract and ​also how to use ​the no arbitrage principle to price forward contracts. ​"
...
- A specified amount of an asset (i.e., delivery of stock, sale of gold, $ or EUR, delivery of a future Tbill of a given maturity etc)
...
Value of the contract at time T is known: $f_T=(S_T-F)$
- long position in forward: must purchase the asset at price F
- can immediately spot price of asset: $S_T$
- If long position value spot > forward buyin, then profit
Forward price F is set so that time t=0 value/price $f_0$ is 0
Use no-arbitrage principle to set F. Buyer and seller are indiff between long or short the contract.
...
\end{aligned}$$
You construct the hedge of the long contract by shorting to remove the randomness inherent in $S_T$ of the future stock price. $d(0,T)$ is just the discount factor; dividing by the discount factor will grow it by the 1+r compounded up to T based on the term structure.
...
F=\frac{S_0}{d(0,T)}
\end{aligned}$$
Why is F strictly greater than the spot price $S_0$? Cost of carry. So long as discount < 0 and no convenience yield. In this case forward > spot.
...
f_t = (F_t - F_0)d(t, T)
\end{aligned}$$
The solution to price the forward in the middle is to construct a new forward to hedge it. Portfolio with two different forward contracts, $F_0$ original contract, and a $F_t$ is the new forward, to cancel random quantity spot at maturity.